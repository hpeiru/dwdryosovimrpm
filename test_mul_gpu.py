...
parser.add_argument('--test_dir',default='/home/zhoujiayu/datasets/University-Release/test',type=str, help='./test_data')
parser.add_argument('--name', default='two_view_long_share_d0.75_256_s1_lr0.001_lpn4_balance_yosovim_rpmring', type=str, help='save model path')
...
if opt.multi:
    ...
    if scale_test:
        ...
        dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=False, num_workers=16) for x in ['gallery_satellite', 'gallery_drone', 'query_satellite', 'query_drone']}
...
def extract_feature(model,dataloader, view_index = 1):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            # 先跑一遍输出获取特征维度
            input_img = img.cuda()
            sample_output = model(input_img)
            feat_dim = sample_output.shape[1]

            # 初始化累计特征向量
            ff = torch.zeros(n, feat_dim).cuda()
...
# 禁用所有 classifier 的 logits 输出
for i in range(opt.block):
    getattr(model, f'classifier{i}').classifier = nn.Sequential()
    getattr(model.rpm_branch, f'classifier_rpm_{i}').classifier = nn.Sequential()

model.global_branch.classifier_gap.classifier = nn.Sequential()
model.global_branch.classifier_gmp.classifier = nn.Sequential()
model = model.eval()
if use_gpu:
    model = model.cuda()
